...
var Sinaloa = ee.FeatureCollection('GeoVida/ee-rtk2031126034/assets/12_UTM');
var  Campeche = ee.FeatureCollection('GeoVida/ee-rtk2031126034/assets/16_UTM');
var  NayaSina = ee.FeatureCollection('GeoVida/ee-rtk2031126034/assets/Zona13UTMV1');
var  Campeche2 = ee.FeatureCollection('GeoVida/ee-rtk2031126034/assets/Zona15UTMV1');
...
// Nuevas funciones para crear enlaces (style)
...
// Crear el panel principal con estilo mejorado (botones)
...
//editar despues para tipo de cultivo
var selectStyle = {
  width: '100%',
  margin: '10px 0',
};
...
function actualizarMapa() {
    ...
    for (var i = 0; i < periodos.length; i++) {
      ...
      for (var indice in formulas) {
        var indicePeriodo = calcularIndice("COPERNICUS/S2_SR", fechaInicio, fechaFin,
                                           i === 1 ? 15 : 15, // Menor nubosidad en el período de mayo a agosto (puntos)
                                           BANDAS,
                                           formulas[indice]);
  ...
  // Seleccionar la geometría de la zona (combinacion de las zonas)
  var zonaGeometria;
  var zonaDescripcion;
  switch(zona) {
    case '12UTM':
  ...
  onClick: function() {
    ...
    switch(zona) {
      ...
      case '15UTM':
        ...
        break;
      default:
        return; // No hacer nada si no se ha seleccionado una zona válida (agregar todos los datos necesarios)
    }
    mapPanel.centerObject(zonaGeometria, 8);
  },
  style: {
...
mapPanel.setCenter(-102.5528, 23.6345, 6); // Centrar en la zona del pais de México
